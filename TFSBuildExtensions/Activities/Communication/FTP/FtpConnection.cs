//-----------------------------------------------------------------------
// <copyright file="FtpConnection.cs">(c) http://TfsBuildExtensions.codeplex.com/. This source is subject to the Microsoft Permissive License. See http://www.microsoft.com/resources/sharedsource/licensingbasics/sharedsourcelicenses.mspx. All other rights reserved.</copyright>
//-----------------------------------------------------------------------
namespace TfsBuildExtensions.Activities.Communication
{
    ...
    {   
        ...
        private static string InternetLastResponseInfo(ref int code)
        {
            ...
            int result = NativeMethods.InternetGetLastResponseInfo(ref code, buff, ref buffersize);
            return (result == 0) ? buff.ToString() : string.Empty;
        }
