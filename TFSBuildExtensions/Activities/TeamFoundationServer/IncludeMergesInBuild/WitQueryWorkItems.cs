...
namespace TfsBuildExtensions.Activities.TeamFoundationServer.IncludeMergesInBuild
{
    ...
    public sealed class WitQueryWorkItems : AsyncCodeActivity<WorkItemCollection>
    {
        ...
        {
            this.PageSize = 50;
            this.IncludeParentWorkItems = false;
        }
        ...
        {
            var func = new Func<WorkItemStore, IEnumerable<int>, IEnumerable<string>, int, bool, Tuple<WorkItemCollection, List<WorkItemLinkInfo>>>(this.RunCommand);
            context.UserState = func;
            return func.BeginInvoke(this.WorkItemStore.Get(context), this.Ids.Get(context), this.Fields.Get(context), this.PageSize.Get(context), this.IncludeParentWorkItems.Get(context), callback, state);
        }
        ...
        protected override WorkItemCollection EndExecute(AsyncCodeActivityContext context, IAsyncResult result)
        {
            ...
            this.ParentChildLinks.Set(context, tuple.Item2);
            return items;
        }
        ...
        private Tuple<WorkItemCollection, List<WorkItemLinkInfo>> RunCommand(WorkItemStore workItemStore, IEnumerable<int> ids, IEnumerable<string> fields, int pageSize, bool includeParentWorkItems)
        {
            ...
            if (includeParentWorkItems && ids.Count() > 0)
            {
                ...
                var query = string.Format("SELECT [System.Id] FROM WorkItemLinks WHERE ([System.Links.LinkType] = 'System.LinkTypes.Hierarchy-Forward') And ([Target].[System.Id] IN ({0})) ORDER BY [System.Id] mode(Recursive,ReturnMatchingChildren)", idArray);
